...
class DialogMessageConsumer(mixins.CreateModelMixin,
    ...
    async def start_queue(self):
        ...
        while self.queue:
            ...
            data, status = await self.create_dialog_message(message= await self.queue[0][2]['message'], recipient= await self.queue[0][2]['recipient'], request_id= await self.queue[0][2]['request_id'], action= await self.queue[0][2]['action'])
