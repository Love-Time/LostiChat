...
class DialogMessageConsumer(mixins.CreateModelMixin,
    ...
    async def start_queue(self):
        ...
        while self.queue:
            print(self.queue)
            instance, data = await self.create_dialog_message2(message=self.queue[0][1]['message'], recipient=self.queue[0][1]['recipient'], request_id=self.queue[0][2]['request_id'], action=self.queue[0][1]['action'])
