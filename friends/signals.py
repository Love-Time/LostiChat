...
def create_user_channel(sender, instance: Friends, created, **kwargs):
    ...
    serializer = UserSimpleSerializer(instance)
    if not another_request and instance.accepted == 1:
        # Сказать второму, что ему отправили запрос в друзья
        async_to_sync(channel_layer.group_send)(f'recipient_{instance.second_user}',
                                                {"type": "send_message", "data": {'type': 'friend_request',
    ...
    if instance.accepted == 1 and another_request == 1:
        ...
        async_to_sync(channel_layer.group_send)(f'recipient_{instance.second_user}',
                                                {"type": "send_message", "data": {'type': 'friend_accepted',
    ...
    if instance.accepted == -1 and another_request == 1:
        ...
        async_to_sync(channel_layer.group_send)(f'recipient_{instance.second_user}',
                                                {"type": "send_message", "data": {'type': 'friend_denied',
...
def create_user_channel(sender, instance: Friends, created, **kwargs):
    ...
    if not another_request and instance.accepted == 0:
        ...
        async_to_sync(channel_layer.group_send)(f'recipient_{instance.second_user}',
                                                {"type": "send_message", "data": {'type': 'friend_canceled',
    ...
    if another_request[0].accepted == 1 and instance.accepted == 1:
        ...
        async_to_sync(channel_layer.group_send)(f'recipient_{instance.second_user}',
                                                {"type": "send_message", "data": {'type': 'friend_delete',
